...
post_json() {
  local path="$1"
  local body="$2"
  # /ingest_chunk & friends are CSRF-protected (server/security.py): a
  # state-changing POST without the X-Estormi-Origin header is rejected 403
  # before reaching the route. Send the same value first-party connectors use
  # (estormi_ingestion/shared/http_client.py) so the seed actually ingests.
  curl -sS -X POST "$BASE$path" \
    -H "Content-Type: application/json" \
    -H "X-Estormi-Origin: estormi-connector" \
    -d "$body"
}
...
if [ "$FAIL" -eq 0 ]; then
  ...
  # Dump the server log here: the EXIT trap cleans up the temp dir, so just
  # printing its path is useless in CI where nobody can open the file after.
