...
flowchart TD
    ...
    TagCmd --> CreateTag["3. Create git tag v1.8 & Push"]:::step
    
    CreateTag -->|Triggers release.yml| CIRun["4. CI Release Pipeline"]:::step
...
The `tag` target (see [`Makefile`](../Makefile)) creates the tag and pushes the
branch + the tag. The push to `v*` is what triggers the release workflow
([`.github/workflows/release.yml`](../.github/workflows/release.yml)).

The README download badge needs no regeneration: it is a live
[shields.io](https://shields.io) endpoint
(`img.shields.io/github/v/release/francoisdeverdun/Estormi`) that reads the
latest GitHub release, so it tracks the new tag automatically once the release
...
(it asserts the four are identical, so drift fails the gate). Two further
surfaces are derived tracks, not part of that four-file version:

| Location                          | Today's value | Notes                                         |
| --------------------------------- | ------------- | --------------------------------------------- |
| `packages/estormi_server/__init__.py` | `0.0.3`  | `__version__` — the Python source of truth; FastAPI `version=` and the MCP `serverInfo.version` both forward it. |
| `apps/estormi-macos/tauri.conf.json`       | `0.0.3`       | Bundle / app version surfaced in macOS About. |
| `apps/estormi-macos/Cargo.toml`            | `0.0.3`       | Rust crate version.                           |
| `pyproject.toml`                  | `0.0.3`       | `[project].version`. One of the four files `make set-version` keeps in lockstep. |
...
| README download badge             | latest release | Derived live: a shields.io endpoint reading the latest GitHub release — no committed file, nothing to bump. |

Run `make set-version V=X.Y.Z` to bump the four declaring files together before
running `make tag`. `build_version.txt` is written from the git tag/SHA by
`make build-version` / `make tag`, and the download badge is resolved by
shields.io at render time — neither is set by hand.

## Badges

[`assets/badges/`](../assets/badges/) holds three QA SVG badges rendered into
the README:
...
They regenerate from the latest `build/coverage/coverage.json` via `make
test-metrics` (kept out of `make test` so local runs don't dirty the tree). The
README download badge is not a file here — it is a live shields.io endpoint that
reads the latest GitHub release, so it tracks the tag with no regeneration step.
