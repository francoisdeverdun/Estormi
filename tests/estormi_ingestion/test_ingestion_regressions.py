...
class TestDocumentsICloudPlaceholderWalk:
    ...
    def _walk_candidates(self, root: Path) -> set[str]:
        ...
        with (
            ...
            # The .icloud placeholder remap fires `brctl download` via
            # subprocess.Popen (not .run); without mocking it too, the real
            # macOS-only `brctl` is invoked and crashes on Linux CI.
