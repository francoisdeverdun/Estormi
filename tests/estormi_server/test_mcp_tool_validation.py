...
async def test_missing_required_argument(client):
    ...
    assert "query" in body["error"]["message"].lower() or "required" in body["error"]["message"].lower()


@pytest.mark.unit
async def test_wrong_type_argument(client):
    """search_memory.limit must be integer; a string should yield -32602."""
    resp = await client.post("/mcp", json=_rpc("search_memory", {"query": "hi", "limit": "not_int"}))
    body = resp.json()
    assert body["error"]["code"] == -32602
